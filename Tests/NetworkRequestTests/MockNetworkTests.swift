...
class MockNetworkTests: XCTestCase {
    ...
    func testFailureMock() throws {
        ...
        let request = ItemRequest()
        networkService.mock(request: request, result: .failure(.apiError(Data("test".utf8), HTTPURLResponse())))
        
        assertNetworkResponse(service: networkService, request: request, expectedResult: .failure(.apiError(Data("test".utf8), HTTPURLResponse())))
        assertNetworkResponse(service: networkService, request: ItemRequest2(), expectedResult: .failure(.noResponse))
    }
